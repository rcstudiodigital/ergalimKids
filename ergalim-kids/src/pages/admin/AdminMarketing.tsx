...
const TEMPLATES: Record<Template, { icon: string; label: string; subject: string; title: string; message: string; buttonText: string }> = {
  promo: {
    icon: '🏷️', label: 'Promoção',
    subject: '🏷️ Promoção especial na Ergalim Kids!',
    ...
    <div className="space-y-6 animate-fadeUp max-w-4xl">
      ...
      <div>
        <h1 className="text-2xl font-black text-white flex items-center gap-2">
          <Megaphone size={22} className="text-brand-pink"/> E-mail Marketing
        </h1>
        <p className="text-sm text-gray-400 font-bold mt-1">
          Envie promoções, novidades e cupons para seus clientes
        </p>
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4 flex items-center gap-3">
        ...
        <div className="flex-1">
          {loadingCustomers ? (
            ...
            <>
              <p className="text-sm font-black text-white">{customers.length} cliente(s) cadastrado(s)</p>
              <p className="text-xs text-gray-500 font-bold">A campanha será enviada para todos</p>
            </>
          )}
        </div>
      </div>
      ...
      {/* Templates */}
      <div>
        <label className="text-xs font-black text-gray-400 uppercase tracking-wider mb-2 block">Escolha um modelo</label>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {(Object.keys(TEMPLATES) as Template[]).map(t => (
            <button key={t} onClick={() => applyTemplate(t)}
              className={`p-4 rounded-2xl border-2 text-center transition-all ${template===t ? 'border-brand-pink bg-brand-pink/10' : 'border-gray-700 hover:border-gray-500'}`}>
              <div className="text-2xl mb-1">{TEMPLATES[t].icon}</div>
              <p className={`text-xs font-black ${template===t ? 'text-brand-pink' : 'text-gray-300'}`}>{TEMPLATES[t].label}</p>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 space-y-4">
        <div>
          <label className="text-xs font-black text-gray-400 block mb-1">Assunto do e-mail *</label>
          <input value={form.subject} onChange={e => setForm(f=>({...f,subject:e.target.value}))}
            className="input-field bg-gray-800 border-gray-700 text-white"
            placeholder="🏷️ Promoção especial!"/>
        </div>
        <div>
          <label className="text-xs font-black text-gray-400 block mb-1">Título (dentro do e-mail) *</label>
          <input value={form.title} onChange={e => setForm(f=>({...f,title:e.target.value}))}
            className="input-field bg-gray-800 border-gray-700 text-white"
            placeholder="Promoção Imperdível!"/>
        </div>
        <div>
          <label className="text-xs font-black text-gray-400 block mb-1">Mensagem *</label>
          <textarea value={form.message} onChange={e => setForm(f=>({...f,message:e.target.value}))} rows={4}
            className="input-field bg-gray-800 border-gray-700 text-white resize-none"
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          <div>
            <label className="text-xs font-black text-gray-400 block mb-1">Cupom (opcional)</label>
          ...
          <div>
            <label className="text-xs font-black text-gray-400 block mb-1">Texto do botão</label>
            <input value={form.buttonText} onChange={e => setForm(f=>({...f,buttonText:e.target.value}))}
              className="input-field bg-gray-800 border-gray-700 text-white" placeholder="Ver promoções"/>
          </div>
        </div>

        {/* Imagem */}
        <div>
          <label className="text-xs font-black text-gray-400 block mb-1">Imagem de destaque (opcional)</label>
          <ImageUpload value={form.imageUrl} onChange={url => setForm(f=>({...f,imageUrl:url}))} folder="ergalim-kids/campaigns"/>
        </div>
      </div>
      ...
      {result && (
        <div className="bg-green-900/30 border border-green-700 rounded-2xl p-4 flex items-center gap-3">
          ...
          <div>
            <p className="font-black text-green-300">Campanha enviada com sucesso!</p>
            <p className="text-xs text-green-400/70 font-bold">{result.sent} enviados · {result.failed} falharam</p>
          </div>
        </div>
      )}
      ...
      <div className="flex gap-3">
        <button onClick={() => setShowPreview(!showPreview)} className="btn-outline border-gray-600 text-gray-300 flex items-center gap-2">
          <Eye size={16}/> {showPreview ? 'Ocultar' : 'Pré-visualizar'}
        </button>
        <button onClick={handleSend} disabled={sending || customers.length === 0}
          className="btn-primary flex-1 flex items-center justify-center gap-2 py-3.5">
          {sending
            ? <><Loader2 size={16} className="animate-spin"/> Enviando...</>
            : <><Send size={16}/> Enviar para {customers.length} cliente(s)</>}
        </button>
      </div>

      {/* Preview */}
      {showPreview && (
        <div className="bg-white rounded-2xl overflow-hidden border-4 border-gray-700">
          <div className="bg-gradient-to-r from-brand-navy to-blue-800 p-8 text-center">
            <div className="text-2xl font-black text-white">⭐ ergalim <span className="text-brand-pink">kids</span></div>
          </div>
          {form.imageUrl && <img src={form.imageUrl} alt="" className="w-full max-h-48 object-cover"/>}
          <div className="p-6">
            <h2 className="text-xl font-black text-brand-navy mb-3">{form.title || 'Título da campanha'}</h2>
            <p className="text-sm text-gray-600 leading-relaxed mb-4">{form.message || 'Sua mensagem aparecerá aqui...'}</p>
            {form.couponCode && (
              <div className="bg-bg-soft border-2 border-dashed border-brand-pink rounded-2xl p-4 text-center mb-4">
                <p className="text-xs font-bold text-gray-400 uppercase">Seu cupom</p>
                <p className="text-2xl font-black text-brand-pink tracking-wider my-1">{form.couponCode}</p>
                {form.couponDiscount > 0 && <p className="text-sm font-bold text-brand-navy">{form.couponDiscount}% OFF! 🎉</p>}
              </div>
            )}
            <div className="text-center">
              <span className="inline-block bg-brand-pink text-white font-black px-8 py-3 rounded-2xl">{form.buttonText} →</span>
            </div>
          </div>
        </div>
      )}

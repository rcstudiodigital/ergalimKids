        ...
        <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4">
          <div className="w-full max-w-2xl bg-white rounded-2xl shadow-2xl animate-fadeUp max-h-[90vh] flex flex-col">
            ...
            <div className="flex border-b border-gray-100 px-5">
              {([['info','Informações'],['images','Imagens'],['variants','Tamanhos & Estoque']] as const).map(([t, label]) => (
                <button key={t} onClick={()=>setTab(t)}
                  className={`px-4 py-3 text-sm font-bold border-b-2 transition-colors ${tab===t ? 'border-brand-pink text-brand-pink' : 'border-transparent text-gray-400 hover:text-gray-600'}`}>
                  {label}
                </button>
              ))}
            </div>
            ...
            <div className="p-5 overflow-y-auto flex-1">
              ...
              {tab === 'info' && (
                <div className="space-y-3">
                  <div><label className="text-xs font-bold text-gray-500 block mb-1">Nome do produto *</label>
                    <input value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} className="input-field" placeholder="Ex: Conjunto Moletom Feminino Rosa"/></div>
                  <div className="grid grid-cols-2 gap-3">
                    <div><label className="text-xs font-bold text-gray-500 block mb-1">Preço (R$) *</label>
                      <input type="number" step="0.01" min="0" value={form.price||''} onChange={e=>setForm(f=>({...f,price:parseFloat(e.target.value)||0}))} className="input-field" placeholder="189,90"/></div>
                    <div><label className="text-xs font-bold text-gray-500 block mb-1">Preço original (R$)</label>
                      <input type="number" step="0.01" min="0" value={form.originalPrice||''} onChange={e=>setForm(f=>({...f,originalPrice:parseFloat(e.target.value)||undefined}))} className="input-field" placeholder="239,90 (opcional)"/></div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div><label className="text-xs font-bold text-gray-500 block mb-1">Subcategoria</label>
                      <input value={form.subcategory||''} onChange={e=>setForm(f=>({...f,subcategory:e.target.value}))} className="input-field" placeholder="Ex: Conjuntos, Moletons..."/></div>
                  </div>
                  <div><label className="text-xs font-bold text-gray-500 block mb-1">Descrição</label>
                    <textarea value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))} rows={3} className="input-field resize-none" placeholder="Descreva o produto detalhadamente..."/></div>
                  <div className="flex items-center gap-4 pt-1">
                    <label className="flex items-center gap-2 cursor-pointer text-sm font-semibold">
                      <input type="checkbox" checked={form.featured} onChange={e=>setForm(f=>({...f,featured:e.target.checked}))} className="rounded accent-pink w-4 h-4"/> ⭐ Produto em destaque
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer text-sm font-semibold">
                      <input type="checkbox" checked={form.active} onChange={e=>setForm(f=>({...f,active:e.target.checked}))} className="rounded accent-pink w-4 h-4"/> Produto ativo (visível na loja)
                    </label>
                  </div>
                </div>
              )}
              ...
              {tab === 'images' && (
                <div className="space-y-4">
                  <p className="text-sm text-gray-500 font-bold">Adicione até 5 imagens. A primeira é a capa do produto.</p>
                  {form.images.map((url, i) => (
                    <div key={i} className="p-3 bg-gray-50 rounded-2xl border border-gray-200">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-black text-gray-500">{i === 0 ? '📸 Imagem principal (capa)' : `📷 Imagem ${i + 1}`}</span>
              ...
              {tab === 'variants' && (
                <div className="space-y-4">
                  <p className="text-sm text-gray-500 font-bold">Adicione os tamanhos, cores e quantidades em estoque. <span className="text-brand-pink">Obrigatório pelo menos um.</span></p>
                  <div className="grid grid-cols-4 gap-2 text-xs font-bold text-gray-400 px-1">
                    <span>Tamanho *</span><span>Cor *</span><span>Estoque</span><span></span>
                  </div>
                  {form.variants.map((v, i) => (
                    <div key={i} className="grid grid-cols-4 gap-2 items-center p-3 bg-gray-50 rounded-xl">
                      <input value={v.size} onChange={e => updateVariant(i, {size: e.target.value})} className="input-field text-sm py-2" placeholder="Ex: 4, P, M"/>
                      <input value={v.color} onChange={e => updateVariant(i, {color: e.target.value})} className="input-field text-sm py-2" placeholder="Ex: Rosa"/>
                      <input type="number" min="0" value={v.stock} onChange={e => updateVariant(i, {stock: parseInt(e.target.value)||0})} className="input-field text-sm py-2"/>
                      <button onClick={() => removeVariant(i)} disabled={form.variants.length === 1} className="text-red-400 hover:text-red-600 disabled:opacity-20 p-1 justify-self-center"><X size={16}/></button>
                    </div>
                  ))}
                  <button onClick={addVariant} className="btn-outline w-full flex items-center justify-center gap-2 text-sm"><Plus size={15}/> Adicionar tamanho / cor</button>
                  <div className="p-3 bg-gray-50 rounded-xl">
                    <p className="text-xs font-bold text-gray-500 mb-2">Atalhos de tamanho:</p>
                    <div className="flex flex-wrap gap-2">
                      {[['Roupas infantis','2,4,6,8,10,12'],['PP,P,M,G,GG','PP,P,M,G,GG']].map(([label,sizes]) => (
            ...
            <div className="flex gap-3 p-5 border-t border-gray-100">
              ...
              <button onClick={handleSave} className="btn-primary flex-1">{editProduct ? 'Salvar alterações' : 'Adicionar produto'}</button>
            </div>
          </div>
        </div>,
        document.body

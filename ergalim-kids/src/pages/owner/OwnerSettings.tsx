...
export default function OwnerSettings() {
  ...
  return (
    <div className="space-y-6 animate-fadeUp max-w-2xl">
      <h1 className="text-2xl font-black text-brand-navy flex items-center gap-2"><Store size={22} className="text-brand-pink" /> Configurações da Loja</h1>

      <div className="card p-5 space-y-4">
        <h2 className="font-bold text-brand-navy flex items-center gap-2"><Store size={16} /> Informações Básicas</h2>
      ...
      <div className="card p-5 space-y-4">
        ...
        {/* Instagram */}
        <div>
          <label className="text-xs font-black text-gray-500 block mb-1">📸 Instagram</label>
          ...
          {form.storeInstagram && (
            <a
              ...
              className="inline-flex items-center gap-1.5 mt-2 text-xs font-black text-brand-pink hover:underline">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <div>
              <input
                ...
                className="input-field"
                placeholder="(24) 99239-1998"
              />
              <p className="text-xs text-gray-400 font-bold mt-1">Exibido na loja</p>
            </div>
            <div>
              <input
                ...
                placeholder="5524992391998"
              />
              <p className="text-xs text-gray-400 font-bold mt-1">Só números (DDI+DDD+número)</p>
            </div>
          </div>
          {form.storeWhatsapp && (
            <a
              ...
              target="_blank" rel="noreferrer"
              className="inline-flex items-center gap-1.5 mt-2 text-xs font-black text-green-600 hover:underline">
              💬 wa.me/{form.storeWhatsapp} ↗
            </a>
          )}
        </div>

        <div className="p-3 bg-bg-blue border border-blue-200 rounded-xl text-xs text-gray-500 font-bold">
          💡 Após salvar, o botão do WhatsApp e o link do Instagram já atualizam em toda a loja.
        </div>
      </div>

      <div className="card p-5 space-y-4">
        <h2 className="font-bold text-brand-navy flex items-center gap-2"><Globe size={16} /> Conteúdo do Site</h2>
        {F('Título do hero (banner principal)', 'heroTitle')}
        {F('Subtítulo do hero', 'heroSubtitle')}
      </div>

      <div className="card p-5 space-y-4">
        <h2 className="font-bold text-brand-navy flex items-center gap-2"><Bell size={16} /> Notificações por E-mail</h2>
      ...
      <button onClick={save} className="btn-primary w-full flex items-center justify-center gap-2 py-3.5 text-base">

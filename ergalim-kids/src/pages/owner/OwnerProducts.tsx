                  ...
                  <tr key={p.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => toggleFeatured(p)} className={`btn-ghost p-1.5 ${p.featured?'text-amber-500':''}`} title="Destaque">
            ...
            <div className="p-5 overflow-y-auto flex-1">
              ...
              {activeTab === 'info' && (
                <div className="space-y-4">
                  <div>
                    <label className="text-xs font-bold text-gray-500 block mb-1">Nome do produto *</label>
                    <input value={form.name} onChange={e => setForm(f=>({...f,name:e.target.value}))} className="input-field" placeholder="Ex: Conjunto Moletom Feminino Rosa"/>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-xs font-bold text-gray-500 block mb-1">Preço (R$) *</label>
                      <input type="number" step="0.01" min="0" value={form.price||''} onChange={e => setForm(f=>({...f,price:parseFloat(e.target.value)||0}))} className="input-field" placeholder="189,90"/>
                    </div>
                    <div>
                      <label className="text-xs font-bold text-gray-500 block mb-1">Preço original (R$)</label>
                      <input type="number" step="0.01" min="0" value={form.originalPrice||''} onChange={e => setForm(f=>({...f,originalPrice:parseFloat(e.target.value)||undefined}))} className="input-field" placeholder="239,90 (opcional)"/>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                    ...
                    <div>
                      <label className="text-xs font-bold text-gray-500 block mb-1">Subcategoria</label>
                      <input value={form.subcategory} onChange={e => setForm(f=>({...f,subcategory:e.target.value}))} className="input-field" placeholder="Ex: Conjuntos, Moletons..."/>
                    </div>
                  </div>
                  <div>
                    <label className="text-xs font-bold text-gray-500 block mb-1">Descrição</label>
                    <textarea value={form.description} onChange={e => setForm(f=>({...f,description:e.target.value}))} rows={4} className="input-field resize-none" placeholder="Descreva o produto detalhadamente..."/>
                  </div>
                  <div className="flex items-center gap-5">
                    <label className="flex items-center gap-2 cursor-pointer text-sm font-semibold text-gray-700">
                      <input type="checkbox" checked={form.featured} onChange={e => setForm(f=>({...f,featured:e.target.checked}))} className="rounded accent-pink w-4 h-4"/>
                      ⭐ Produto em destaque
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer text-sm font-semibold text-gray-700">
                      <input type="checkbox" checked={form.active} onChange={e => setForm(f=>({...f,active:e.target.checked}))} className="rounded accent-pink w-4 h-4"/>
              ...
              {activeTab === 'variants' && (
                <div className="space-y-4">
                  <p className="text-sm text-gray-500">Adicione os tamanhos, cores e quantidades em estoque.</p>
                  ...
                  {form.variants.map((v, i) => (
                    <div key={i} className="grid grid-cols-4 gap-2 items-center p-3 bg-gray-50 rounded-xl">
                      <input value={v.size} onChange={e => updateVariant(i, {size: e.target.value})}
                        className="input-field text-sm py-2" placeholder="Ex: 4, P, M"/>
                      <input value={v.color} onChange={e => updateVariant(i, {color: e.target.value})}
                        className="input-field text-sm py-2" placeholder="Ex: Rosa"/>
                      <input type="number" min="0" value={v.stock}
                        onChange={e => updateVariant(i, {stock: parseInt(e.target.value)||0})}
                        className="input-field text-sm py-2"/>
                      <button onClick={() => removeVariant(i)} disabled={form.variants.length === 1}
                        className="text-red-400 hover:text-red-600 disabled:opacity-20 p-1 justify-self-center">
                        <X size={16}/>
                      </button>
                    </div>
                  ))}

                  <button onClick={addVariant} className="btn-outline w-full flex items-center justify-center gap-2 text-sm">
                    <Plus size={15}/> Adicionar tamanho / cor
                  </button>

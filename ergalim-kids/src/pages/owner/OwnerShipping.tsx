    ...
    <div className="space-y-6 animate-fadeUp max-w-3xl">
      ...
      <div className="card p-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleSaveFreeAbove} className="btn-navy">Salvar</button>
        </div>
        <p className="text-xs text-gray-400 mt-2 flex items-center gap-1">
          <AlertCircle size={12} /> Atual: frete grátis nas compras acima de {formatCurrency(settings.freeShippingAbove)}
        </p>
      </div>
        ...
        <div className="divide-y divide-gray-50">
          {shippingOptions.map(opt => (
            <div key={opt.id} className="px-5 py-4">
              {editingId === opt.id && editForm ? (
                /* Formulário de edição inline */
                <div className="space-y-3 animate-fadeUp">
                  <div className="grid sm:grid-cols-2 gap-3">
                    <div>
                      <label className="text-xs font-bold text-gray-500 block mb-1">Nome da opção *</label>
                      <input value={editForm.name} onChange={e => setEditForm({ ...editForm, name: e.target.value })} className="input-field py-2 text-sm" placeholder="Ex: PAC, SEDEX, Motoboy..." />
                    </div>
                    <div>
                      <label className="text-xs font-bold text-gray-500 block mb-1">Prazo estimado *</label>
                      <input value={editForm.estimatedDays} onChange={e => setEditForm({ ...editForm, estimatedDays: e.target.value })} className="input-field py-2 text-sm" placeholder="Ex: 5 a 10 dias úteis" />
                    </div>
                  </div>
                  <div className="grid sm:grid-cols-2 gap-3">
                    <div>
                      <label className="text-xs font-bold text-gray-500 block mb-1">Preço (0 = grátis)</label>
                      <div className="relative">
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">R$</span>
                        <input type="number" min="0" step="0.01" value={editForm.price} onChange={e => setEditForm({ ...editForm, price: parseFloat(e.target.value) || 0 })} className="input-field py-2 text-sm pl-9" />
                      </div>
                    </div>
                    <div>
                      <label className="text-xs font-bold text-gray-500 block mb-1">Descrição / Observação</label>
                      <input value={editForm.description} onChange={e => setEditForm({ ...editForm, description: e.target.value })} className="input-field py-2 text-sm" placeholder="Ex: Apenas para Volta Redonda" />
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <div className="flex items-center gap-1">
                      <button onClick={() => handleToggle(opt)} className="btn-ghost p-1.5 text-gray-400" title={opt.active ? 'Desativar' : 'Ativar'}>
                        {opt.active ? <ToggleRight size={20} className="text-green-500" /> : <ToggleLeft size={20} />}
                      </button>
                      <button onClick={() => { setEditingId(opt.id); setEditForm({ ...opt }) }} className="btn-ghost p-1.5" title="Editar">
                        <Pencil size={15} />
                      </button>
                      <button onClick={() => handleDelete(opt.id, opt.name)} className="btn-ghost p-1.5 hover:text-red-500" title="Remover">
                        <Trash2 size={15} />
                      </button>
                    </div>
                  </div>
          ...
          <div className="border-t border-gray-100 p-5 bg-gray-50 animate-fadeUp">
            ...
            <div className="space-y-3">
              <div className="grid sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-bold text-gray-500 block mb-1">Nome da opção *</label>
                  <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="input-field py-2 text-sm" placeholder="Ex: PAC, SEDEX, Motoboy, Retirada..." />
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-500 block mb-1">Prazo estimado *</label>
                  <input value={form.estimatedDays} onChange={e => setForm({ ...form, estimatedDays: e.target.value })} className="input-field py-2 text-sm" placeholder="Ex: 5 a 10 dias úteis" />
                </div>
              </div>
              <div className="grid sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-bold text-gray-500 block mb-1">Preço (0 para grátis)</label>
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">R$</span>
                    <input type="number" min="0" step="0.01" value={form.price} onChange={e => setForm({ ...form, price: parseFloat(e.target.value) || 0 })} className="input-field py-2 text-sm pl-9" />
                  </div>
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-500 block mb-1">Descrição / Restrição</label>
                  <input value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} className="input-field py-2 text-sm" placeholder="Ex: Apenas para região de Volta Redonda" />
                </div>
              </div>
              <div className="flex gap-2 pt-1">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-sm text-blue-700">
        ...
        <p>Opções com preço <strong>R$0,00</strong> aparecem como "Grátis" para o cliente. Você pode desativar temporariamente uma opção sem precisar excluí-la.</p>
      </div>
    </div>
  )
}

  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      <nav className="flex items-center gap-1.5 text-xs text-gray-400 mb-6">
        <Link to="/" className="hover:text-navy">Início</Link><ChevronRight size={12} />
        <Link to="/shop" className="hover:text-navy">Loja</Link><ChevronRight size={12} />
        <Link to={`/shop?category=${product.category}`} className="hover:text-navy">{product.category}</Link><ChevronRight size={12} />
      ...
      <div className="grid lg:grid-cols-2 gap-10">
        ...
        <div className="flex gap-3">
          <div className="flex flex-col gap-2">
            {product.images.map((src, i) => (
              <button key={i} onClick={() => setImg(i)} className={`w-14 h-16 rounded-xl overflow-hidden border-2 transition-colors ${img===i?'border-pink':'border-transparent'}`}>
        ...
        {/* Info */}
        <div>
          <p className="text-xs text-pink font-black tracking-widest uppercase mb-2">{product.category} · {product.subcategory}</p>
          <h1 className="font-display font-black text-3xl text-navy mb-3">{product.name}</h1>
          ...
          {/* Cor */}
          <div className="mb-5">
            <p className="text-sm font-bold text-gray-700 mb-2">Cor: <span className="text-navy">{color || 'Selecione'}</span></p>
            <div className="flex flex-wrap gap-2">
              {colors.map(c => (
                <button key={c} onClick={() => { setColor(c); setSize(null) }} className={`px-4 py-2 text-sm font-bold border-2 rounded-xl transition-colors ${color===c?'bg-navy text-white border-navy':'border-gray-200 text-gray-700 hover:border-navy'}`}>{c}</button>
              ))}
            </div>
          </div>
          ...
          {/* Tamanho */}
          <div className="mb-6">
            <p className="text-sm font-bold text-gray-700 mb-2">Tamanho: <span className="text-navy">{size || 'Selecione'}</span></p>
            <div className="flex flex-wrap gap-2">
              {sizes.map(s => {
                const v = product.variants.find(v => v.size === s && v.color === (color || colors[0]))
                const out = v && v.stock === 0
                return (
                  <button key={s} disabled={!!out} onClick={() => !out && setSize(s)} className={`w-12 h-10 text-sm font-bold border-2 rounded-xl transition-colors ${size===s?'bg-navy text-white border-navy':out?'border-gray-100 text-gray-300 cursor-not-allowed line-through':'border-gray-200 text-gray-700 hover:border-navy'}`}>{s}</button>
                )
              })}
            </div>
          </div>
          ...
          <button onClick={handleAdd} className="btn-pink w-full py-4 text-base mb-4">
          ...
          <div className="border-2 border-gray-100 rounded-2xl p-4 space-y-3">
            {[
              ...
              <div key={text} className="flex items-center gap-3 text-sm text-gray-600">
                <Icon size={16} className="text-pink shrink-0" />{text}
              </div>
            ))}
          </div>
        </div>
